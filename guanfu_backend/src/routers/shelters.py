...
@router.get("/", response_model=schemas.ShelterCollection)
def list_shelters(
...
@router.post("/", response_model=schemas.Shelter, status_code=201)
def create_shelter(
...
@router.patch("/{id}", response_model=schemas.Shelter)
def patch_shelter(
