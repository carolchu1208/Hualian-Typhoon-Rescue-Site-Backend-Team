...
@router.get("/", response_model=schemas.SupplyItemCollection)
...
@router.post("/", response_model=schemas.SupplyItem, status_code=201)
def create_supply_item(
    ...
    if not parent_supply:
        raise HTTPException(status_code=400, detail=f"Supply with id {item_in.supply_id} does not exist.")
    return crud.create(db, models.SupplyItem, obj_in=item_in)


@router.get("/{id}", response_model=schemas.SupplyItem)
